const CACHE='gds-cache-v7';
const ASSETS=['./','./index.html?v=7','./styles.css?v=7','./manifest.json','./icon-192.png','./icon-256.png','./icon-384.png','./icon-512.png','./icon-512-maskable.png','./apple-touch-icon.png'];
...
self.addEventListener('fetch',e=>{
 ...
 if(acc.includes('text/html')){
  e.respondWith(fetch(req).then(res=>{const copy=res.clone(); caches.open(CACHE).then(c=>c.put(url.pathname+url.search,copy)); return res;})
    .catch(()=>caches.match(url.pathname+url.search)||caches.match('./index.html?v=7'))); return;
 }
